...
class JokeToComicRequest(BaseModel):
    joke: str = Field(min_length=1, max_length=2000)
    confirmed_characters: list[CharacterCandidate] = Field(default_factory=list)


class AnalyzeSeriesCharactersRequest(BaseModel):
    source_text: str = Field(min_length=1, max_length=8000)
    series_name: str = Field(default="", max_length=120)


class AnalyzeSeriesCharactersResponse(BaseModel):
...
class JokeToComicResponse(BaseModel):
    title: str
    source_joke: str
