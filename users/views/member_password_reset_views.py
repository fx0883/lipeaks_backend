...
class MemberPasswordResetRequestPageView(FormView):
    ...
    支持URL参数: ?tenant_id=1 （自动填充并隐藏租户选择框）
    """
    template_name = 'members/password_reset_request.html'
    form_class = MemberPasswordResetRequestForm
    success_url = reverse_lazy('users:members:password-reset-request-sent')
    ...
    def get_form_kwargs(self):
        ...
        if tenant_id:
            kwargs['initial_tenant_id'] = tenant_id
        return kwargs
    
    def get_context_data(self, **kwargs):
        """添加tenant_id到上下文，用于模板判断"""
        context = super().get_context_data(**kwargs)
        context['url_tenant_id'] = self.request.GET.get('tenant_id')
        return context
    ...
    def form_valid(self, form):
        ...
        if request_count >= 13:
            messages.error(self.request, '请求过于频繁，请稍后再试')
        ...
        if member:
            try:
                ...
                reset_url = self.request.build_absolute_uri(
                    reverse('users:members:password-reset-form', kwargs={'token': token})
                )
...
多租户用户管理系统团队
                ...
                if 'reset_token' in locals():
                    reset_token.delete()
                messages.error(self.request, '发送邮件失败，请稍后再试')
                return self.form_invalid(form)
        else:
            # 不暴露用户是否存在，记录日志用于调试
...
class MemberPasswordResetFormView(FormView):
    ...
    success_url = reverse_lazy('users:members:password-reset-complete')
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            messages.error(self.request, '密码重置失败，请重试')
            return self.form_invalid(form)
